...
let html = PageWrap.wrap www.``static``.styles.css {
 ...
 contents = [
  ...
  p [] [
   ul [] [
    li [] [ Text $"""✅ CLI interface only, no need for GUI""" ]
    li [] [ Text $"""✅ Works natively on Unix-like systems""" ]
    li [] [ Text $"""✅ Has many clients for mainstream OS""" ]
    li [] [ Text $"""✅ Only famous FOSS tools (used utils: {co "git"}, {co "pass"}, {co "gpg"}, {co "openssh"}, {co "passphrase2pgp"})""" ]
    li [] [ Text $"""✅ No Google/Facebook account or other BS integration, your credentials are yours and stored by you on your computer""" ]
    li [] [ Text $"""✅ Knowing your primary password is not enough to access your passwords""" ]
    li [] [ Text $"""✅ Still can sync among devices""" ]
    li [] [ Text $"""✅ You control where your backup stored (and it's encrypted too)""" ]
    li [] [ Text $"""❌ That's 100x more complicated to set up than most crappy password managers you'd find on the internet""" ]
   ]
  ]
