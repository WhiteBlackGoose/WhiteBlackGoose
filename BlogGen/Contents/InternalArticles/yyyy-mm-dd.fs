...
let page = {
    ...
    contents = [
        p [] [
            ...
            *Disclaimer: the arguments here are mostly for people who read from left to right*
            "
        ]
        br []
        hr []
        ...
        p [] [
            Text "
            Our universal format allows to *cut* any continuous interval from ISO8601 (**YYYY-MM-DD HH:mm:ss**) which still makes perfect sense. For example, you want to hold a party soon, so you cut (YYYY-**MM-DD HH**:mm:ss). Or you want to research when Napoleon started working on his Napoleonic plans, which would be (**YYYY-MM**-DD HH:mm:ss). Or a lecture starts soon, you're perhaps interested in (YYYY-MM-DD HH:**mm:ss**). Likewise, a full date and a full time are also continuous intervals taken from this universal format!
            "
        ]
        ...
        p [] [
            Text "
            Which one happened earlier: **10/5/12** or **7/8/12**? Yeah... first of all, is it mm/dd/yy or dd/mm/yy or something else? For either of them, 
            you will have to think by which of the sections to compare first.
            "
        ]
        p [] [
            Text "
            Now, compare **2012-05-10** and **2012-07-08**. If you read from left to right, all you need to do is to find the leftmost different digit
            and compare them. Clearly, **2012-07-08** comes after **2012-05-10**, because 7 is greater than 5, and you don't need to pad zeros,
            go over the date in a random order of \"sections\". You do it **exactly** as you do it with numbers, time, and other measurements.
            "
        ]
        p [] [
            Text "
